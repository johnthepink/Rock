...
namespace Rock.Net
{
    public class LinkParser
    {
        ...
        public void ParseLinks( CrawledPage page, string sourceUrl, string startUrl )
        {
            ...
            for ( int i = 0; i <= matches.Count - 1; i++ )
            {
                ...
                {
                    if ( IsExternalUrl( foundHref, startUrl ) )
                    {
                        _externalUrls.Add( foundHref );
                    }
                    else if ( !IsAWebPage( foundHref ) )
                    {
                        foundHref = Crawler.FixPath( sourceUrl, foundHref, startUrl );
